...
exports.criarBem = async (req, res) => {
    try {
        ...
        console.log('Valor de etiqueta recebido:', etiqueta); 
...
exports.adcionarLevantamento = async (req,res) => {
    const {data_hora, responsavel, ano} = req.body;
    try {
        const bem = await bemModel.adcionarLevantamento( data_hora, responsavel, ano);
        res.status(200).send(bem);
    } catch (erro) {
        console.error('Erro no controlador:', erro);
...
exports.editarBem = async (req, res) => {
    try {
        ...
        const { idbem, nome, codigo, numero, estado_conservacao, valor_aquisicao, data_aquisicao, categoria_idCategoria, local_idLocais } = req.body;
        console.log("contrlee: ");
        const resultado = await bemModel.editarBem(idbem, nome, codigo, numero, estado_conservacao, valor_aquisicao, data_aquisicao, categoria_idCategoria, local_idLocais);
        res.status(200).send(resultado);
    } catch (erro) {
        res.status(500).send("controler", erro);
    }
}
...
exports.listarBensNoTag = async (req, res) => {
    try{
        const bens = await bemModel.listarBensNoTag();
        res.status(200).json(bens);
    } catch (erro) {
        res.status(500).send(erro);
        console.log(erro)
...
exports.buscarPorNome = async (req, res) => {
    try {
        const nome = req.query.nome; // Obtém o parâmetro de pesquisa da URL
        const bens = await bemModel.buscarPorNome(nome);
        res.status(200).json(bens);
    } catch (erro) {
        res.status(500).send(erro);
        console.log(erro);
...
exports.criarLocal = async (req, res) => {
    try {
        ...
        const local = await bemModel.criarlocal(nome, responsavel);
...
exports.listarLocal = async (req, res) => {
    try {
        const { idLocal } = req.params;
        console.log('ID Local recebido:', idLocal); // Verifique o valor de idLocal
        const listarLocais = await bemModel.listarIDLocais(idLocal);
